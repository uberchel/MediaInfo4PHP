 ...
 */
 ...
 class MediaInfo {
     ...
     * @return Object or Array
     */
    public static function Get(string $file = '', bool $mdata = false, bool $toObject = true) {
        // check file on empty
     ...
     * @return void
     */
    public static function Load (string $file = '', bool $isObject = true) {
        // check file on empty
        ...
        foreach ($streams[0] as $item) {
            ...
            if (preg_match('/Stream #(\d+:\d+)(?:\(([^\)]+)\))?: ([^\n]+)/', $item, $main)) {
                ...
                if (stripos($stream['type_info'], 'Video:') === 0) {
                    $stream['type'] = stripos($stream['type_info'], 'attached pic') ? 'pictures' : 'videos';
                    ...
                    if (preg_match('/BPS\s+: (\d+)/m', $item, $bpsMatch)) {
    ...
    private static function FFMpegExec (string $file) {
        ...
        exec ("ffmpeg -i {$file} 2>&1", $result, $retval);
